...
function Sidenav({ color, brand, brandName, routes, ...rest }: Props): JSX.Element {
  ...
  const renderNestedCollapse = (collapse: any) => {
    const template = collapse.map(({ name, route, key, type, href }: any) =>
      href ? (
        <Link
          key={key}
          href={href}
  ...
  const renderRoutes = routes.map(
    ({ type, name, icon, title, collapse, noCollapse, key, href, route }: any) => {
      let returnValue;
      const permissionKey = permissionRoute[key];
      const isAllowed = authUser.isAuthenticated?
        authUser?.permission[permissionKey] ||
        permissionKey === "all" ||
        key === "home" ||
        key == "username":false;
        
      if (type === "collapse" && isAllowed) {
        if (href) {
          returnValue = (
            <Link
              href={href}
          ...
          returnValue = (
            <SidenavCollapse
              key={key}
              // name={name}
              name={name == "UserName" ? `${authUser?.firstName} ${authUser?.lastName}` : name}
              icon={icon}
              active={key === collapseName}
